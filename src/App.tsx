  ...
  const fetchConversationsBetweenUsers = async (
    ...
    try {
      const response = await fetch(
        ...
        `http://localhost:1337/api/conversations?filters[participants][id][$in]=${loggedInUserId}&filters[participants][id][$in]=${clickedUserId}&populate[participants][populate]=*&populate[1]=messages&populate[messages][populate]=senders`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${jwt}`,
